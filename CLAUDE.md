...
- Do not add message strings to `assert*` calls in tests (e.g. `assertNull(x)` not `assertNull(x, "message")`).
- When fixing a bug: write the test first, verify it fails with the old behavior, then implement the fix and verify the test passes. Do not start implementing until the failing test exists.
- Do not manually alter files in `src/test/resources/refdata/refdata/` — all content there is generated by `make import-test-resources`.
